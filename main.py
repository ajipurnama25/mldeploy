...
def predict():
    ...
    model_dict = { 'CNN Model'     :   'static/MLModule/model_cnn5_tkc.h5'}
    #model_dict = {'InceptionResNetV2 Model' : 'static/MLModule/model_ModelInceptionResNetV2.h5'}
    if chosen_model in model_dict:  
        model = load_model(model_dict[chosen_model]) 
    else:
        model = load_model(model_dict[0])
    file = request.files["file"]
    file.save(os.path.join('static', 'blb.jpg'))
    ...
    return predict_result(chosen_model, runtimes, respon_model, calculation , 'blb.jpg')

def predict_result(model, run_time, probs, result, img):
    class_list = {'Bacterial leaf blight': 0, 'Brown spot': 1, 'Leaf smut': 2}
